# AI SDLC Assistant Platform — Implementation Plan v3 (Final)
...
| # | File Path | Purpose |
|---|-----------|---------|
| 1 | `docker-compose.yml` | Postgres 15 (+ pgvector extension), Temporal (server + UI), Redis (rate limiting + caching) |
...
- Rate limiting via `throttle.guard.ts` backed by Redis (`@fastify/rate-limit`)
...
### Redis Usage
- **Rate limiting:** Backing store for `@fastify/rate-limit` (distributed rate limiting across API instances)
- **Caching:** Future use for agent output caching, MCP response caching
- Not used for job queues (Temporal handles workflow orchestration)

### API ↔ Frontend Contract Sharing
- Frontend `api-client.ts` imports Zod schemas from `libs/shared/schemas/` for response validation
- Provides runtime type safety without a separate code-generation step
- Future option: add `@nestjs/swagger` to generate OpenAPI spec for external consumers
...
- Backing services (Postgres, Redis, Temporal) treated as attached resources
